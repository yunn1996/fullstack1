...
import {toastSuccess, toastError} from "../api/ToasService";
...
const ContactDetail = ({updateContact, updateImage}) => {
    ...
    const updatePhoto = async (file) => {
        try {
            ...
            console.log(file);
            const {data: updatedPhotoUrl} = await updateImage(formData);
            console.log(updatedPhotoUrl);
            setContact((prev) => (
                {
                    ...prev,
                    photoUrl: updatedPhotoUrl ? `${updatedPhotoUrl}?updated_at=${new Date().getTime()}` : prev.photoUrl
                }
            ));
            toastSuccess("Update photo successfully!")
        } catch (error) {
            console.error(error);
        }
    }
    ...
    const onUpdateContact = async (event) => {
        ...
        try {
            ...
            toastSuccess("Update contact successfully!")
        } catch (e) {
            toastError(e);
        }
    }
            ...
            <div className='profile'>
                <div className='profile__details'>
                    ...
                    <div className='profile__metadata'>
                        ...
                        <p className='profile__muted'>PNG. Max size of 10MG</p>
